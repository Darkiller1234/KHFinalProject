...
public class MembershipController {
	...
	public ModelAndView loginMember(Member m, HttpSession session, ModelAndView mv, String saveId,
...
//	            : null;
			...
			// 네이버 로그인 회원정보 확인
			if (memberService.socialMemberCheck(id) == 0) {
				...
				session.setAttribute("socialRegi", naverM);

				return "member/socialRegi";
			} else {
				Member loginMember = memberService.getSocialMember(id);
	...
	@RequestMapping("login/socialJoin")
	public String socialJoin(Member m, HttpSession session, Model model) {

		Member social = (Member) session.getAttribute("socialRegi");
		m.setMemberId(social.getMemberId());
		m.setMemberPwd(social.getMemberPwd());
		m.setSocial(social.getSocial());
		m.setSocialId(social.getSocialId());

		// 회원가입 처리
		int result = memberService.socialJoin(m);

		// 결과에 따라 알림 메시지 설정 및 페이지 이동
		if (result > 0) {

			Member loginMember = memberService.getSocialMember(social.getSocialId());
			session.setAttribute("loginMember", loginMember);
			return "redirect:/";
		} else {
			model.addAttribute("errorMsg", "회원가입 실패");
	...
	@Value("${sns.kakao.clientSecret}")
	private String kakaoclientSecret;

	@RequestMapping("login/kakao")
	public String kakaoLoginCallback(String code, String state, String error, HttpSession session) throws Exception {
		if (error != null) {
			return "redirect:/";
		}
		String redirectURL = URLEncoder.encode("http://localhost:5600/T3B1/member/login/kakao", "UTF-8");
...
//        }
		...
		if (responseCode == 200) {
			...
			log.info("result : {}", memberInfo.getAsJsonObject("response"));
			log.info("msg : {}", memberInfo.getAsJsonObject("msg"));
...
//	            : null;
			...
			if (memberService.socialMemberCheck(id) == 0) {
				...
				kakaoM.setMemberId(result2);

				kakaoM.setMemberPwd("kakao");
...
//				}
				...
				session.setAttribute("socialRegi", kakaoM);
				...
				session.setAttribute("loginMember", loginMember);
				log.info("gkgkgk" + Integer.toString(responseCode));
